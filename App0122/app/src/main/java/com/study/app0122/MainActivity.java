...
public class MainActivity extends AppCompatActivity {
    ...
    //게시물 출력하기  (서버로 부터 받은 String(json형식) 데이터를 넘겨받아 출력할 예정)
    public void printData(String data){
        Log.d(TAG,"핸들러로 부터 전달받은 데이터는 "+data);
        ...
        //재사용 하기 위해 미리 정의해놓은 레이아웃 파일을 인플레이션 시켜본다
        ...
        for(int i=0;i<memberList.size();i++){
            ...
            //아래의 인플레이션이 발생한 후, profile_item.xml의 최상위
            //레이아웃이 반환된다.  root 는  MainActivity속 wrapper 얘야.
            ViewGroup root_wrapper = (ViewGroup)layoutInflater.inflate(R.layout.profile_item,wrapper);
            Log.d(TAG,"인플레이션 결과 뷰 그룹은 "+root_wrapper);
            ...
            Log.d(TAG,"profile_root 리니어는 "+text_root);
    ...
    public void loadData(View view){
        ...
        //통신 쓰레드를 동작시키자!!
        //쓰레드는 하나의 프로세스내에서 독립적으로(또한 비동기적 특징도 있음음) 실행되는,또하나의 세부 실행단위
        ConnectManager manager = new ConnectManager(this,"http://192.168.1.2:8888/rest/member",null);
        manager.start();
