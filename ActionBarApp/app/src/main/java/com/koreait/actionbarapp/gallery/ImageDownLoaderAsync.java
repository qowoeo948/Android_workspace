...
//매 요청마다 객체가 1:1 대응됨..
//대부분 <URL,진행상황을 알 수 있는 숫자값(Void,Integer),String  >
public class ImageDownLoaderAsync extends AsyncTask<String,Void, Bitmap> {
    ...
    //웹서버 요청 전에 하고 싶은 작업 (메인쓰레드 영역, 따라서 UI제어 가능영역)
    protected void onPreExecute() {
        super.onPreExecute();
    }

    //웹서버 요청 수행(개발자 정의 쓰레드가 수행, 따라서 UI제어 불가)
    ...
    //웹서버 요청 도중에 하고싶은 작업..메인쓰레드가 수행 따라서 UI제어 가능)
    protected void onProgressUpdate(Void... values) {
        super.onProgressUpdate(values);
    }

    //웹서버 요청 완료 후에 하고싶은 작업은 여기서..(메인쓰레드가 수행 따라서 UI제어 가능)
