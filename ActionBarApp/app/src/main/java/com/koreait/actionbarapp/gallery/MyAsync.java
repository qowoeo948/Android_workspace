...
public class MyAsync extends AsyncTask<String, Void, String> {
    ...
    //doInBackground()메서드에서의 반환값이 아래의 onPostExecute(O) 메서드의 인수로 전달 (시스템에 의해 자동으로)
    protected void onPostExecute(String data) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                ImageDownLoaderAsync downAsunc = new ImageDownLoaderAsync(gallery,galleryFragment);
                downAsunc.execute("http://192.168.1.2:7777/images/"+gallery.getFilename());
            }
