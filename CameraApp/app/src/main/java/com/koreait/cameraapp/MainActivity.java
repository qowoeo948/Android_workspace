...
public class MainActivity extends AppCompatActivity {
    private static final int CAMERA_REQUEST=1; //내가만든 요청코드
    ImageView imageView;
    ...
    public void openCamera(View view){
        //다른앱을 호출하려면, 암시적 인텐트를 이용해야한다! 왜?
        //그 앱을 제작한 개발자가 만든 액티비티명을 모르기 떄문에...
        //또한, 다른 앱을 접근할 수 없기 떄문에 안드로이드 시스템에게
        //간접적으로 열어줄것을 부탁해야 한다.

        /*2015년 마시멜로 버전이후부터는 보안이 강화되었기 떄문에
        AndroidManifest.xml에 퍼미션을 명시하더라도,보안취득을 인정하지 않는다.
        위험한(Dangerous)권한은 사용자에게 허락에 대한 요청을 시도하도록 바뀜..
        * */

        //권한을 취득한 경우만 카메라앱을 호출해야한다.
    ...
    public void gotoConfig(){
        ...
        intent.setData(uri);    //기존 Intent에 uri 추가!
        startActivity(intent);
    }

    //요청 후 결과를 가져올게 있을 떄 이 메서드 재정의..(카메라앱 or 갤러리 앱...모른다..)
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == CAMERA_REQUEST){  //카메라 앱으로부터 온 응답이라면..
            ...
            Bitmap bitmap =(Bitmap)bundle.get("data");   //이미 정해진 키값인 data를 대입한다.
