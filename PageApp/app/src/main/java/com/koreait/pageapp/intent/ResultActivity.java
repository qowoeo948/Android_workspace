...
public class ResultActivity extends AppCompatActivity {
    ...
    public void send() {
        //보낼 데이터 구성하기
        Intent intent = new Intent();//명시하지 않은 이유? 우리가 원하는건 새롭게 생성된 액티비티가 아니다,즉 기존 액티비티와 소통해야함
        ...
        //마치 http통신에서 서버코드(200,500 등)처럼..
        setResult(this.RESULT_OK,intent);    //나를 호출한 액티비티에 결과 전달하는 메서드
        //나를 호출한 액티비티가 누군지 시스템이 알고 있다. 따라서 이 메서드만 호출하면 됨됨

        //내가 사라져야 다른 화면이 나올수 있음
        finish();
    }
